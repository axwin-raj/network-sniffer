...
def resource_path(relative_path: str, base: Optional[str] = None) -> str:
    """
    Return absolute path to a resource, works in dev and PyInstaller builds.
    - In a PyInstaller exe, sys._MEIPASS points to the temp bundle dir.
    - In dev, base defaults to the current working directory.
    """
    if getattr(sys, "_MEIPASS", None):
        base_path = sys._MEIPASS
    else:
        base_path = base or os.path.abspath(".")
            ...
            with open(filename, "w", newline="", encoding="utf-8") as f:
                ...
                for idx, pkt in enumerate(packets, start=1):
                    row = {
                        ...
                        "protocol": pkt.get("protocol", ""),  # <-- ensure this line matches exactly
